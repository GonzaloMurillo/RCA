...
class ReplicationContextPlot():
  ...
  def plot_context(self,lrepl_client_time_stats,save_name):
      ...
      _log.debug("Save name:{}".format(self.save_name))
      _log.debug("Lrep client time stats received by the class ReplicationContextPlot {}".format(lrepl_client_time_stats))
      ...
      for i,iterator_list in enumerate(lrepl_client_time_stats):
          _log.debug("List number:{}".format(i))
          _log.debug("key:{}|value:{}".format(iterator_list['key'],iterator_list['value']))
      ...
      _log.debug("Num of keys:{}".format(num_keys))
      ...
      _log.debug("send_refs_percentage:{}".format(send_refs_percentage))
    ...
    # We 'explode' in the graph the greatest value, bt we need to find it first and build the explode tuple
      ...
      _log.debug("We are going to save the graph:{}".format(self.save_name))
  ...
  def random_name(self,lenght_of_name):
      ...
      for i in range(0,lenght_of_name):
          ...
          _log.debug("the random number:{}".format(random_number))
