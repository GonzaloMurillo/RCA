                ...
                frontend_structe[ctx_num]['suggestedFix'] = [
                    {
                        ...
                        'action_item': {
                            ...
                            'list_of_steps': [  # Empty list if not needed
                                'Check for the existance of RPC Errors in the ddfs.info logs, and if your found them, follow the steps described here: <a href="https://support.emc.com/kb/466384">Replication Troubleshooting (Internal)</a><br>',
                                'Verify that any throttle schedule is correct and that the values for the throttles are correct.For reference check: <a href="https://support.emc.com/kb/306046">Replication Throttle</a><br>',
                ...
                frontend_structe[ctx_num]['suggestedFix'] = [
                    {
                        ...
                        'action_item': {
                            'one_liner': 'We have been unable to identify a predominand bottleneck. ',
                            'list_of_steps': [  # Empty list if not needed
                            ],
                            'footnote': '' # Blank string if not needed
                        },
                        'details': 'We have been unable to identify a predominand bottleneck.<br>This means that there is not a single factor causing replication problems on this mtree replication context.<br><br><strong>How you should proceed?</strong> Consider using delta difference option to upload more than one autosupport, and to check what this context has been doing lately.<br>Perhaps there is a predominant bottleneck lately that can be detected by the delta difference calculation.<br><br>It might be also that two operations are taking the highest percentage of time. Then you should address each of the issues separately.<br>For example: it can be a combined issue where local reading is slow, but also the network bandwidth is also insufficient.<br><br>For further information about how RCA works, please <a href="https://confluence-dev4.gtie.dell.com/x/WZGeB">click here</a>'}
                    # This is a list, so we can have multiple suggested fixes for the same context, if applicable
                ]
